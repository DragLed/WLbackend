...
class WishlistAccess(Base):
    ...
    user = relationship(
        "User", foreign_keys=[user_id], back_populates="wishlist_accesses"
    )

    role: Mapped[WishlistRole] = mapped_column(
        Enum(WishlistRole), nullable=False, name="role"
    )

    invited_by = relationship(
        "User", foreign_keys=[invited_by_id], back_populates="invited_accesses"
    )
