...
def create_wishlist(wl: WishlistBase, visibility:WishlistVisibility, db: Session = Depends(get_db), token: dict = Depends(security.access_token_required)):
    """
    Create a wishlist.
    """
    return WishlistInterface.create_wishlist(db, wl, token.sub, visibility)

@rout.get("/", dependencies=[Depends(security.access_token_required)], response_model=list[WishlistRespone])
def get_my_wishlists(db: Session = Depends(get_db), token: dict = Depends(security.access_token_required)):
...
def delete_wishlist(id:int, db: Session = Depends(get_db), token: dict = Depends(security.access_token_required)):
    return WishlistInterface.delete_wishlist(db, id, token.sub)
